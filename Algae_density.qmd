...
As we already know, our data are around `r zeros`% 0s, and we a quick fall-off of count frequency in a typical negative binomial distribution.
...
summary(invert_model_region)$coefficients[2,] %>% kable()

confint(invert_model_region, "total_percent") %>% kable()
```
